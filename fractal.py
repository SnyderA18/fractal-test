...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.off_screen_image = PIL.Image.new("RGB", (SCREEN_WIDTH, SCREEN_HEIGHT), (0,0,0))
    ...
    def on_draw(self):
        ...
        # arcade.draw_texture_rectangle(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, self.texture.width, self.texture.height, self.texture)
        # self.texture.draw(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, self.texture.width, self.texture.height)
    ...
    def update(self, delta_time):
        ...
        color = (i << 21) + (i << 10) + i*8
        self.bitmap[self.x, self.y] = color
        print(self.x, self.y, "#%06x" % color)
        # for y in range(min(SCREEN_HEIGHT, SCREEN_WIDTH)):
        #     self.bitmap[self.x,y] = color
        self.texture = arcade.Texture('background', self.off_screen_image)
        self.background_sprite.texture = self.texture
